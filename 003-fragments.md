# Ecto fragment/1 example

Today we are going to explore how to use Ecto to perform a common SQL operation using fragments.
...
```
Now lets create our schema & migrations. Note, we are also creating out custom type for our duration field, as we want to store our timestamp ranges in a specific format & tsrange is the best way to do that in postgres. But since ecto doesn't have a built in type for tsrange, we need to create our own.
...
#Note that in the above code equal? is used to compare two tsrange values for equality, & embed_as is used to specify that the type should be represented as a string in the database. Both of these are required for our custom type to work.

#Now we begin with applying our migrations
...
```
We have created quite the complex set of schemas, models, & migrations.
Now we can move on to using our fragments & applying it to our queries which conver different use cases & contexts which shall be demonstrated below.
...
# An example of using string interpolation with fragments
...
# Examples demonstrating the usage of a subquery with a fragment & dynamic query building
  ...
  # This demonstrates the use of a macro to create a fragment for calculating duration in hours
...
### Now we can run our queries & check the results
```elixir
  def main do
...
Here we demonstrated the use of fragments in Ecto for various use cases, involving subqueries, string interpolation, dynamic query building, & the use of macros to create reusable fragments. This flexibility allows for powerful and dynamic query construction in Elixir applications using Ecto.